...
namespace SlickQA.SlickSharp.Test
{
	...
	public sealed class JsonObject_Get
	{
		...
		public void List()
		{
			Project project1 = StreamConverter<Project>.ReadFromStream(StreamConversion.FromString(PROJECT_1));
			Project project2 = StreamConverter<Project>.ReadFromStream(StreamConversion.FromString(PROJECT_2));

			const string PROJECT_LIST = "[" + PROJECT_1 + "," + PROJECT_2 + "]";
			MemoryStream listStream = StreamConversion.FromString(PROJECT_LIST);
		...
		{
			MemoryStream project1Stream = StreamConversion.FromString(PROJECT_1);
			Project expectedProject = StreamConverter<Project>.ReadFromStream(project1Stream);
		...
		public void Item_that_does_not_exist_with_createIfNotFound_true()
		{
			...
			using (var requestStream = new MemoryStream())
			{
				...
				_mockRequest.Setup(request => request.GetResponse()).Returns(() => _mockResponse.Object).Callback(() =>
				                                                                                                  {
